...
void motors_task(void *parameter) {
  ...
  for (;;) {
    ...
    if (left_motor_acceleration) {
      left_motor_acceleration = ((double)left_motor_acceleration / 1000.0) *
                                (double)MOTORS_REFRESH_MS;
      ...
      left_motor_speed = left_motor_target_speed;
    }
    ...
    if (right_motor_acceleration) {
      right_motor_acceleration = ((double)right_motor_acceleration / 1000.0) *
                                 (double)MOTORS_REFRESH_MS;
      ...
      right_motor_speed = right_motor_target_speed;
    }
...
void Motors::begin() {
  ...
  if (xQueue_left_motor_speed == NULL) {
    ESP_LOGE(TAG, "Error creating the queue \"xQueue_left_motor_speed\"");
  }

  if (!xQueueSend(xQueue_left_motor_speed, &value, portMAX_DELAY)) {
    ESP_LOGE(TAG, "Error setting the queue \"xQueue_left_motor_speed\"");
  }

  xQueue_right_motor_speed = xQueueCreate(1, sizeof(int16_t));

  if (xQueue_right_motor_speed == NULL) {
    ESP_LOGE(TAG, "Error creating the queue \"xQueue_right_motor_speed\"");
  }

  if (!xQueueSend(xQueue_right_motor_speed, &value, portMAX_DELAY)) {
    ESP_LOGE(TAG, "Error setting the queue \"xQueue_right_motor_speed\"");
  }
  ...
  if (!xQueueSend(xQueue_left_motor_acceleration, &value, portMAX_DELAY)) {
    ESP_LOGE(TAG, "Error setting the queue \"xQueue_left_motor_acceleration\"");
  }
  ...
  if (!xQueueSend(xQueue_left_motor_target_speed, &value, portMAX_DELAY)) {
    ESP_LOGE(TAG, "Error setting the queue \"xQueue_left_motor_target_speed\"");
  }
...
void Motors::motors_speed(int16_t left_motor_speed, int16_t right_motor_speed) {
  this->left_motor_speed(left_motor_speed, 0);
  this->right_motor_speed(right_motor_speed, 0);
}

void Motors::motors_speed_accelerated(int16_t left_motor_speed,
                                      int16_t right_motor_speed,
                                      uint16_t left_motor_acceleration,
                                      uint16_t right_motor_acceleration) {
  this->left_motor_speed(left_motor_speed, left_motor_acceleration);
  this->right_motor_speed(right_motor_speed, right_motor_acceleration);
}
